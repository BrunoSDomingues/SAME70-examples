...
#define NUM_TAPS   12  // ordem do filtro (quantos coeficientes)
...
void task_lcd(void){
  ...
  while (true) {
	x_old = R * cos(2*PI*plot.filtrado/4095) + ILI9488_LCD_WIDTH/2;
	y_old = R * sin(2*PI*plot.filtrado/4095) + ILI9488_LCD_HEIGHT/2;
    ...
    if (xQueueReceive( xQueuePlot, &(plot), ( TickType_t )  100 / portTICK_PERIOD_MS)) {     
	  ...
	  // substitui o circulo vermelho por um circulo preto somente no local anterior, evitando redesenhar todo o circulo
	  ili9488_set_foreground_color(COLOR_CONVERT(COLOR_BLACK));
	  ili9488_draw_filled_circle(x_old, y_old, 2);
	  
	  // captura o novo local e preenche com o circulo vermelho
      ili9488_set_foreground_color(COLOR_CONVERT(COLOR_RED));
      ili9488_draw_filled_circle(R * cos(2*PI*plot.filtrado/4095) + ILI9488_LCD_WIDTH/2, R * sin(2*PI*plot.filtrado/4095) + ILI9488_LCD_HEIGHT/2, 2);
    }
  }    
}
