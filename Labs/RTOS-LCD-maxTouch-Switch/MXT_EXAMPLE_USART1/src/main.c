...
typedef struct {
	...
	uint8_t status;		// status do botão
	void (*callback)(t_but);
	tImage image;
} t_but;
...
int process_touch(t_but botoes[], touchData touch, uint32_t n){
	printf("y = %d \n", touch.y);
	for (int i = 0; i < n; i++){
		uint32_t y_min = botoes[i].y;
		uint32_t y_max = botoes[i].y + botoes[i].height;
		uint32_t x_min = botoes[i].x - botoes[i].width/2;
		uint32_t x_max = botoes[i].x + botoes[i].width/2;
		
		if (touch.x >= x_min && touch.x <= x_max)
			if (touch.y >= y_min && touch.y <= y_max) return i;
	}
	return -1;
}
...
void task_lcd(void){
  ...
  while (true) {
    if (xQueueReceive( xQueueTouch, &(touch), ( TickType_t )  500 / portTICK_PERIOD_MS)) {
      int b = process_touch(botoes, touch, n_botoes);
      if(b >= 0){
