...
void request_format(char *p, char s[]) {
...
static void task_process(void *pvParameters) {
  ...
  while(1){
    ...
    switch(state){
      ...
      case GET:
      printf("STATE: GET \n");
	  request_format(g_sendBuffer, "/status");
      ...
      if(xQueueReceive(xQueueMsg, &p_recvMsg, 5000) == pdTRUE){
		...
		const char buffer[4] = "led";
		char *retturn;

		retturn = strstr(p_recvMsg->pu8Buffer, buffer);
		...
		if (!led_state) {
			if (strcmp(retturn[7], '1')) {
				pio_set(LED_PIO, LED_IDX_MASK);
			} else if (strcmp(retturn[7], '0')) {
				pio_clear(LED_PIO, LED_IDX_MASK);
			} else {
				printf("O LED nao foi encontrado!");
			}
		} else {
			pio_set(LED_PIO, LED_IDX_MASK);
		}
...
void LED_init(void){
	...
	NVIC_SetPriority(BUT_PIO_ID, BUT_PRIORITY); // Prioridade 4
