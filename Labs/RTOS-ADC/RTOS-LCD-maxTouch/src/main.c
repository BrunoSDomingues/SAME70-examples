...
/** Semaforo g_is_conversion_done */
SemaphoreHandle_t xConversion;
...
static void AFEC_pot_Callback(void){
	...
	xSemaphoreGiveFromISR(xConversion, &xHigherPriorityTaskWoken);
	adcData adc;
...
void task_lcd(void){
  ...
  // criadas
  font_draw_text(&arial_72, "1024", 50, 200, 2);
...
void task_adc(void){
	...
	xConversion = xSemaphoreCreateBinary();
	
	if (xConversion == NULL)
	printf("falha em criar o semaforo \n");

	while(1){
		if( xSemaphoreTake(xConversion, ( TickType_t ) 500) == pdTRUE ){
			printf("%d\n", g_ul_value);
			vTaskDelay(500);
